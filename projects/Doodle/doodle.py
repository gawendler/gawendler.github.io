...
pg.init()
pg.display.set_caption("Doodle jump AI")
...
PLATFORM_WIDTH, PLATFORM_HEIGHT = 65, 10
bg_y = -22500+600

STAT_FONT = pg.font.SysFont(None, 36)
...
class Platform:
    ...
    def __init__(self, color):
        self.y_offset = 300
        self.w = PLATFORM_WIDTH
        self.h = PLATFORM_HEIGHT
        ...
        if self.color == "Red":
            ...
            self.y_offset *= 7
            self.hor_vel = 5 if self.pos.x > self.win_w / 2 else -5
        self.rect = pg.Rect(self.pos.x, self.pos.y, self.w, self.h)

    def moveDown(self, win, pl_y):
        if self.pos.y >= self.win_h:
                self.pos.y = min(pl_y) - self.y_offset if min(pl_y) - self.y_offset < 0 else random.randrange(-self.y_offset, 0)
                self.pos.x = random.randrange(0, scr_size - self.w)
        self.rect.x, self.rect.y = self.pos.x, self.pos.y
        pg.draw.rect(win, (44,62,80), self.rect)
    
    def moveLF(self, win):
        if self.pos.x + self.w >= scr_size or self.pos.x <= 0:
...
class Agent:
    ...
    def __init__(self, x, y):
        ...
        self.rect = pg.Rect(self.pos.x, self.pos.y, self.size, self.size)  # Changed to a square
    ...
    def update_pos(self, win, pl_x, pl_y):
        ...
        pg.draw.rect(win, (44,62,80), self.rect)  # Draw a color agent
        ...
        for con in genome.connections:
            ...
            for layer_id in range(len(node_layers)):
                for node_id in range(len(node_layers[layer_id])):
                    ...
                    offset_pos = -offset_pos
                offset_pos = 0
            pg.draw.line(win,(0,255,0), (round(x_graph+x_offset*node_one_x),round(y_graph+max_offset/2 + y_offset*node_one_y)),(round(x_graph+x_offset*node_two_x),round(y_graph+max_offset/2 + y_offset*node_two_y)), 2)

def update_win(win, platforms, pl_x, pl_y, agents, speed, score, gen, pl_pos, highest_y, current_Genome):
    global bg_y, max_y, max_Score
    win.fill((236, 240, 241))
    ...
    for agent in agents:
        if agent.pos.y < highest_y and agent.jump:
            highest_y = agent.pos.y
            score += 1
            
            for platform in platforms:
                platform.pos.y += agent.jump_force*3
                bg_y += agent.jump_force/20
                if bg_y >= 0:
                    bg_y = -22500/3 
                pl_pos[platforms.index(platform)] = platforms[platforms.index(platform)].pos
                pl_x[platforms.index(platform)]= platforms[platforms.index(platform)].pos.x
        ...
        agent.update_pos(win, pl_x, pl_y)
    
    #Draw Network of the current genome 
    drawNet(win, current_Genome)
    score_label = STAT_FONT.render("Highscore: " + str(max_Score), 1, (44,62,80))
    win.blit(score_label, (10, 10))
    score_label = STAT_FONT.render("Score: " + str(score), 1, (44,62,80))
    win.blit(score_label, (10, 50))
    score_label = STAT_FONT.render("Generation: " + str(gen), 1, (44,62,80))
...
def run(config_file):
    ...
    winner = p.run(run_game, generations_rate)
     # Plot results after training
    generations = list(range(len(average_scores)))

    plt.figure(figsize=(10, 5))
    plt.plot(generations, average_scores, marker='o', linestyle='-', label="Average Score")

    # Add a trend line
    if len(generations) > 1:
        z = np.polyfit(generations, average_scores, 1)  # Linear fit (degree=1)
        p = np.poly1d(z)  # Create a polynomial function
        plt.plot(generations, p(generations), linestyle="--", color="red", label="Trend Line")

    # Labels and Title
    plt.xlabel("Generation")
    plt.ylabel("Average Score")
    plt.title("Generation vs. Average Score")
    plt.legend()
    plt.grid(True)
    plt.show()
...
def run_game(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        agents.append(Agent(450/2, 0))
        
        while agents != [] and not quit_game:
            clock.tick(30)
        
            score = update_win(win, platforms, pl_x, pl_y, agents, speed, score, gen, pl_pos, highest_y, genome)

            # COLLISION

            for agent in agents:

                r_dist = agent.lookInDir("RIGHT",platforms)
                l_dist = agent.lookInDir("LEFT",platforms)
                u_dist = agent.lookInDir("UP",platforms)
                d_dist = agent.lookInDir("DOWN",platforms)
                dr_dist = 0
                dl_dist = 0
                ur_dist = 0
                ul_dist = 0

                if agent.collide(platforms):
                    genome.fitness += 10
                ...
                for pos in pl_pos:
                    distance = math.sqrt((pos.x - agent.pos.x + agent.size/2)**2+(pos.y-agent.pos.y)**2)
                    if distance <= min_dist and pos.y > agent.pos.y:
                        min_dist = distance
                        if agent.pos.x > pos.x:
                            if pos.y > agent.pos.y:
                                ul_dist = min_dist
                            else:
                                dl_dist = min_dist
                        else:
                ...
                if output[0] == maxOut and output[0] > 0.5 :
                    agent.move_horizontal(-1)
                elif output[1] == maxOut and output[1] > 0.5:
                    agent.move_horizontal(1)
                else:
                    agent.move_horizontal(0)
            ...
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    quit_game = True
                    pg.quit()
                # Kill the agent when "Delete" is pressed
                if event.type == pg.KEYDOWN and event.key == pg.K_DELETE:
                    if agents:  # Ensure there are agents before trying to remove one
                        agents.pop()  # Remove the last agent in the list
        if max_Score < score:
            max_Score = score
            max_Gen = gen
            max_Genome = genome_id
        bg_y = -22500+600
        total_score += score  # Sum scores of all genomes in this generation

    avg_score = total_score / num_agents if num_agents > 0 else 0
    average_scores.append(avg_score)
    print(f"Generation {gen}: Avg Score = {avg_score}")
